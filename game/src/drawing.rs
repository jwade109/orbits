...
fn draw_cross<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, p: Vec2, size: f32, color: Srgba) {
...
fn draw_x<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, p: Vec2, size: f32, color: Srgba) {
    let s = size / 2.0;
    gizmos.line_2d(p + Vec2::new(-s, -s), p + Vec2::new(s, s), color);
    gizmos.line_2d(p + Vec2::new(s, -s), p + Vec2::new(-s, s), color);
}

fn draw_square<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, p: Vec2, size: f32, color: Srgba) {
    gizmos.rect_2d(
...
fn draw_diamond<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, p: Vec2, size: f32, color: Srgba) {
    let s = size / 2.0;
    let pts = [0.0, PI / 2.0, PI, -PI / 2.0, 0.0].map(|a| p + rotate(Vec2::X * s, a));
    gizmos.linestrip_2d(pts, color);
}

fn draw_circle<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, p: Vec2, size: f32, color: Srgba) {
    gizmos
        .circle_2d(Isometry2d::from_translation(p), size, color)
        .resolution(200);
}

fn draw_velocity_vec<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    pv: PV,
    length: f32,
    color: Srgba,
...
fn draw_aabb<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, aabb: AABB, color: Srgba) {
    gizmos.rect_2d(Isometry2d::from_translation(aabb.center), aabb.span, color);
}

fn draw_region<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    region: Region,
    color: Srgba,
    origin: Vec2,
...
fn draw_obb<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, obb: &OBB, color: Srgba) {
...
fn draw_orbit<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    orb: &SparseOrbit,
    origin: Vec2,
    color: Srgba,
...
fn draw_planets<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
...
fn draw_propagator<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
...
fn draw_object<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    planets: &PlanetarySystem,
    obj: &Orbiter,
    stamp: Nanotime,
...
fn draw_scenario<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    scenario: &Scenario,
    stamp: Nanotime,
    scale: f32,
...
fn draw_scalar_field_cell<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
...
fn draw_scalar_field<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    scalar_field: &impl Fn(Vec2) -> f32,
    levels: &[i32],
) {
...
fn draw_event_marker_at<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    event: &EventType,
    p: Vec2,
    scale: f32,
...
fn draw_event<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    planets: &PlanetarySystem,
    event: &EventType,
    stamp: Nanotime,
...
fn draw_highlighted_objects<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, state: &GameState) {
    _ = state
        .highlighted()
        .into_iter()
        .filter_map(|id| {
...
fn draw_controller<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    stamp: Nanotime,
    ctrl: &Controller,
    scenario: &Scenario,
...
fn draw_event_animation<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    scenario: &Scenario,
    id: ObjectId,
    stamp: Nanotime,
...
fn draw_maneuver_plan<T: GizmoConfigGroup>(
    gizmos: &mut Gizmos<T>,
    stamp: Nanotime,
    plan: &ManeuverPlan,
    origin: Vec2,
...
fn draw_timeline<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, state: &GameState) {
...
fn draw_scale_indicator<T: GizmoConfigGroup>(gizmos: &mut Gizmos<T>, cam: &CameraState) {
    let width = 300.0;
...
pub struct BelowShadowGizmoConfigGroup;

#[derive(Default, Reflect, GizmoConfigGroup)]
pub struct AboveShadowGizmoConfigGroup;

pub fn update_gizmo_settings(mut config_store: ResMut<GizmoConfigStore>) {
    let (config, _) = config_store.config_mut::<AboveShadowGizmoConfigGroup>();
    config.render_layers = RenderLayers::layer(2);
    let (config, _) = config_store.config_mut::<BelowShadowGizmoConfigGroup>();
    config.render_layers = RenderLayers::layer(0);
}

pub fn draw_above_shadow(mut gizmos: Gizmos<AboveShadowGizmoConfigGroup>, state: Res<GameState>) {
...
pub fn draw_game_state(mut gizmos: Gizmos<BelowShadowGizmoConfigGroup>, state: Res<GameState>) {
...
fn draw_mouse_state<T: GizmoConfigGroup>(mouse: &MouseState, gizmos: &mut Gizmos<T>) {
    let points = [
