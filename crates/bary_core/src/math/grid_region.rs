...
/// Given the root isometry of a grid and a region within that grid,
/// returns the fully propagated isometry of the grid region.
...
mod tests {
    ...
    fn grid_placement_math_north() {
        ...
        assert_eq!(
            gp.origin_isometry(),
            Isometry2d::new((1.75, 0.75), PI / 2.0)
        );

        assert_eq!(
            gp.center_isometry(),
            Isometry2d::new((1.5, 1.125), PI / 2.0)
        );
    }
    ...
    fn grid_placement_math_west() {
        ...
        assert_eq!(gp.origin_isometry(), Isometry2d::new((3.75, 2.25), PI));

        assert_eq!(gp.center_isometry(), Isometry2d::new((3.375, 1.75), PI));
    }
    ...
    fn grid_placement_math_south() {
        ...
        assert_eq!(gp.origin_isometry(), Isometry2d::new((0.5, 4.25), 1.5 * PI));

        assert_eq!(
            gp.center_isometry(),
            Isometry2d::new((1.75, 1.125), 1.5 * PI)
        );
    }
