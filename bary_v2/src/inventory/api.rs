use bary_core::prelude::{InvSlot, PartCoord};
use bevy::prelude::*;
...
pub struct InventoryApi<'w, 's> {
    ...
    containers: Query<'w, 's, (&'static ContainerLocation, &'static InvSlot)>,
}
...
impl<'w, 's> InventoryApi<'w, 's> {
    pub fn get_container(
        &self,
        container: Entity,
    ) -> Result<(&ContainerLocation, &InvSlot), QueryEntityError> {
    ...
    pub fn find_container_at(&self, part: Entity, pos: PartCoord) -> Result<Entity, FailedLookup> {
        ...
        for child in children.iter() {
            let (loc, _) = self.containers.get(child)?;
            if loc.contains(pos) {
                info!("Yay! {:?}", loc);
                return Ok(child);
            }
